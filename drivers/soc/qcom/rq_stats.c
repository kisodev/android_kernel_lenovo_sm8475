...
struct rq_data {
	...
	unsigned char mpctl2[MPCTL_MAX_CMD];
	unsigned char schctl[MPCTL_MAX_CMD];
...
static ssize_t show_mpctl2(struct kobject *kobj,
               struct kobj_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%s\n", rq_info.mpctl2);
}

static ssize_t store_mpctl2(struct kobject *kobj,
               struct kobj_attribute *attr, const char *buf, size_t count)
{
	snprintf(rq_info.mpctl2, MPCTL_MAX_CMD, "%s", buf);
	sysfs_notify(rq_info.kobj, NULL, "mpctl2");
	return count;
}

static struct kobj_attribute mpctl2_attr =
	__ATTR(mpctl2, S_IWUSR | S_IRUSR, show_mpctl2,
			store_mpctl2);

static ssize_t show_schctl(struct kobject *kobj,
               struct kobj_attribute *attr, char *buf)
{
    return snprintf(buf, PAGE_SIZE, "%s\n", rq_info.schctl);
}

static ssize_t store_schctl(struct kobject *kobj,
               struct kobj_attribute *attr, const char *buf, size_t count)
{
    snprintf(rq_info.schctl, MPCTL_MAX_CMD, "%s", buf);
    rq_info.schctl[3] = 0x0;
    sysfs_notify(rq_info.kobj, NULL, "schctl");
    return count;
}

static struct kobj_attribute schctl_attr =
	__ATTR(schctl, S_IWUSR | S_IRUSR, show_schctl,
			store_schctl);
